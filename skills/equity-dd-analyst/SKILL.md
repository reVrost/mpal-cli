...
- Prefer `mpal` first for available MarketPal data: `ticker profile`, `ticker fundamentals`, `ticker financials`, `ticker events`, `ticker insiders`, `ticker ownership`, `ticker bars`, `ticker markov`, `strategy list/show` when a MarketPal signal context is relevant, and `capabilities` to confirm tool availability.
